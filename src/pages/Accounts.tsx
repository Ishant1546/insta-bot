...
export default function Accounts() {
  ...
  return (
    <div className="fade-in-soft relative z-0">
      ...
      <button onClick={() => setOpen(true)} className="btn-primary mb-5">
        ➕ Add Account
      </button>

      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
        {accounts.map((acc) => (
          <div key={acc.id} className="glass-panel p-6 rounded-xl flex flex-col gap-2">
      ...
      {open && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center fade-in-soft z-[999]">
          <div className="glass-panel p-8 w-96 rounded-2xl z-[1000]">
            ...
            <input
              className="w-full p-3 mb-3 rounded-xl bg-slate-800/50 border border-slate-600 outline-none"
              placeholder="Email"
              value={email}
            ...
            <input
              className="w-full p-3 mb-5 rounded-xl bg-slate-800/50 border border-slate-600 outline-none"
              type="password"
